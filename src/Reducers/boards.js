export const defaultState = {
  ...
  export default function boards(state = defaultState, action) {
    console.log(action);
    switch (action.type) {
      case "START_FETCHING_BOARDS":
          return Object.assign({}, state, {
          pending: true,
        });
      case "SUCCESS_FETCHING_BOARDS":
        return Object.assign({}, state, {
          pending: false,
          boardsData : action.payload.boards
        });
      case "FAIL_FETCHING_BOARDS":
        return Object.assign({}, state, {
          pending: false,
        });
      case "START_CREATING_BOARDS":
        localStorage.setItem("token", action.payload.user.token);
        return Object.assign({}, state, {
          creating: true
        });
      case "SUCCESS_CREATING_BOARDS":
        localStorage.removeItem("token");
        return Object.assign({}, state, {
            creating: false,
            boardsData: state.boardsData.push(action.payload)
        });
        case "FAIL_CREATING_BOARDS":
        localStorage.removeItem("token");
        return Object.assign({}, state, {
            creating: true,
        });
